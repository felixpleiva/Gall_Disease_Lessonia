...
setwd("D:/Dropbox/Radboud University/publicaciones/Pedro/datos/")#Defino directorio de trabajo
...
summary(gall.fit2)
# El modelo resultante incluiría Age.stage,Month y Maturity
# y explica
r.squaredGLMM(gall.fit2)
# theoretical 0.7776427 0.7879906 (cerca del 80%)
...
# La validación en modelos binarios es difícil por la propia naturaleza de los datos.
# Los gráficos de validación por defecto no son demasiado útiles. 
...
#Una validación práctica es comprobar si se parecen las predicciones [0,1] 
#a las observaciones (1 ó 0)

comparo<-data.frame(observados = datos$Fitness2, predichos = pred.mat$prob)
comparo[1:20, ]



# ver mas ejemplos (sjPlot)
# https://cran.r-project.org/web/packages/sjPlot/vignettes/plot_model_estimates.html

# Validación del modelo
# La validación en modelos binarios es difícil por la propia naturaleza de los 
#  datos. Los gráficos de validación por defecto no son demasiado útiles:
  
# # Test a posteriori para
#meses
...
png("0.1. Gall prevalence per month.png", height=7, width=9, units="in", res=200)
library(gridExtra)
...
png("0.2. Gall prevalence per maturity.png", height=7, width=9, units="in", res=200)
library(gridExtra)
...
png("0.3. Gall prevalence per stage.png", height=7, width=9, units="in", res=200)
library(gridExtra)
